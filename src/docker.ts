...
function renderDockerDetailRow(svc: ServiceInfo): string {
    ...
    function addField(label: string, value: unknown): void {
        ...
        if (Array.isArray(value)) {
            fields.push(detailField(label, value.map(function(v) { return esc(String(v)); }).join('<br>')));
        } else if (typeof value === 'object') {
            var inner = '';
            for (var k in value as Record<string, unknown>) {
                if ((value as Record<string, unknown>).hasOwnProperty(k)) {
                    inner += detailInline(esc(k), esc(String((value as Record<string, unknown>)[k])));
                }
            }
            if (inner) {
                fields.push(detailField(label, inner));
            }
        } else {
            fields.push(detailField(label, esc(String(value))));
        }
    }
    ...
    if (svc.healthcheck) {
        ...
        if (hc.test) {
            if (Array.isArray(hc.test)) {
                hcHtml += detailInline('test', esc(hc.test.join(' ')));
            } else {
                hcHtml += detailInline('test', esc(String(hc.test)));
            }
        }
        ...
        fields.push(detailField('Healthcheck', '<span class="text-muted">Not configured</span>'));
    }
...
export function loadDockerServices(): void {
    ...
    Promise.all([svcReq, linkReq, covReq]).then(function(res: any[]) {
        ...
        services.forEach(function(s, idx) {
            ...
            if (cov) {
                if (cov.covered) {
                    linksHtml += '<span class="badge bg-success me-1" title="Authelia access rule: ' + esc(cov.policy) + '" style="cursor:pointer" data-action="switch-tab" data-tab="tab-btn-authelia">\uD83D\uDEE1 ' + esc(cov.policy) + '</span>';
                } else {
                    linksHtml += '<span class="badge bg-warning me-1" title="Authelia access rule missing" style="cursor:pointer" data-action="switch-tab" data-tab="tab-btn-authelia">\uD83D\uDEE1 missing</span>';
                }
            }
            if (!linksHtml) {
