...
export function renderNPMProxies(proxies: (ProxyResponse & ApiErrorBody)[], hosts: NPMProxyHost[]): void {
    var tbody = document.getElementById('npm-tbody')!;
    var proxyErr = (proxies as unknown as ApiErrorBody).error;
    if (proxyErr) { tbody.innerHTML = '<tr><td colspan="4" class="text-center text-danger py-3">' + esc(proxyErr) + '</td></tr>'; return; }
    if (!proxies.length && !hosts.length) { tbody.innerHTML = emptyRow(4, 'No proxy hosts found'); return; }
    var hostByDomain: Record<string, NPMProxyHost> = {};
    hosts.forEach(function(h) { if (h.domain_names && h.domain_names.length) hostByDomain[h.domain_names[0]] = h; });
    var rows: string[] = [];
    proxies.forEach(function(p, idx) {
        var full = hostByDomain[p.cname];
        rows.push('<tr class="npm-proxy-row" data-idx="' + idx + '" data-action="toggle-npm-proxy-detail">' + '<td data-label="Domain"><code>' + esc(p.cname) + '</code></td>' + '<td data-label="Instance">' + (p.source_instance_name ? '<span class="badge bg-secondary">' + esc(p.source_instance_name) + '</span>' : '<span class="text-muted">\u2014</span>') + '</td>' + '<td data-label="Container">' + (p.container ? esc(p.container) : '<span class="text-muted">\u2014</span>') + '</td>' + '<td data-label="In Kuma">' + (p.in_kuma ? '<span class="badge bg-success">\u2713 In Kuma</span>' : '<span class="badge bg-secondary">\u2717 Missing</span>') + '</td></tr>');
        if (full) rows.push('<tr class="npm-detail-row" data-idx="' + idx + '" style="display:none"><td colspan="4">' + renderNPMProxyDetail(full) + '</td></tr>');
    });
...
export function loadNPMProxies(): void {
    ...
    Promise.all([summaryReq, detailReq]).then(function(res: [(ProxyResponse & ApiErrorBody)[], NPMProxyHost[]]) { renderNPMProxies(res[0], res[1]); }).catch(function(err: unknown) {
...
function renderNPMProxyDetail(h: NPMProxyHost): string {
    ...
    if (h.locations && h.locations.length) {
        ...
        h.locations.forEach(function(l) { locs += '<div class="ms-3">' + esc(l.path || '/') + ' → ' + esc(l.forward_scheme + '://' + l.forward_host + ':' + l.forward_port) + '</div>'; });
        parts.push(locs);
    }
