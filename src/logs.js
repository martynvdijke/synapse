...
export function isLogsLoaded() { return logsLoaded; }
...
export function levelBadge(level) {
    var map = { 'DEBUG': 'bg-secondary', 'INFO': 'bg-primary', 'WARN': 'bg-warning text-dark', 'ERROR': 'bg-danger' };
    return '<span class="badge ' + (map[level] || 'bg-secondary') + '">' + level + '</span>';
}

export function durationStr(ns) {
    if (!ns || ns <= 0) return '';
...
export function renderLogRow(e, isNew) {
...
export function toggleLogMeta(row) {
    var ts = row.getAttribute('data-log-ts');
    var metaRow = row.parentNode.querySelector('.log-meta-row[data-log-idx="' + ts + '"]');
...
export function loadLogs(append) {
    ...
    apiFetch('/api/logs?' + params.toString()).then(function(r){return r.json();}).then(function(entries) {
        if (append) {
        ...
        document.getElementById('log-load-more-area').style.display = entries.length < logsLimit ? 'none' : '';
        logsOffset += entries.length;
        logsLoaded = true;
    }).catch(function(err) { if (err.message === 'not authenticated') return; if (!append) tbody.innerHTML = emptyRow(6, 'Failed to load logs'); });
}

export function connectLogSSE() {
